...
def search_specific_extension():
	...
	if ans_1 in ('Y','y'):
		...
		for entry in os.listdir(root_folder):
			if Path.joinpath(root_folder,entry).is_file() and entry not in ('log.txt','exception.txt'): # We don't want to move log and exception log files in the move. They need to be in the root folder.
				...
				if os.path.exists(new_file_path):
					if Path(Path.joinpath(new_file_path,entry)).exists():
						exception_list.append(f'A file with name: {entry} already exists in {new_file_path}.\nNo action will be taken on this file.\n')
						...
						moved_list.append(f' Moved "{entry}" from <{root_folder}> to <{new_file_path}>.')
					...
					moved_list.append(f'\n "{new_file_path}" created.\n Moved "{entry}" from <{root_folder}> to <{new_file_path}>.')
			else:
				moved_list.append(f'\n "{entry}" is not a file, no action will be taken on it.\n')
