...
def download_video(video_name, channel_name, url, yt, stream, download_video_window):
    ...
    final_file = Path(download_path,f'{channel_name} - {video_name}.mp4') # Changing filename to now have the entire path, the channel name, and the mp4 extension.
    print(f'\n {final_file}')
    ...
    if Path(final_file).exists():
        ...
        try:
            ...
            sg.popup(f'Your video: "{video_name}" has been downloaded and saved at - "{download_path}"', keep_on_top=True)
...
def mains():
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED or 'Exit'):
            ...
            fetch_details_window.close()
            print(video_name)
            download_video_layout = [   [sg.T(f'Your video is "{video_name}"', s=(50,1), pad=(20,10))],
            ...
            if event=='Download':
                download_video(video_name, channel_name, url, yt, stream, download_video_window)
