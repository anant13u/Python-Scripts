...
layout = [  [selectFolderText, folderBrowse], 
            [sg.B('Generate List', s=(15,2), p=((250,120),10)), sg.B('Rename Files', s=(15,2), p=(10,10)), sg.B('Exit', s=(15,2), p=(120,10)), sg.B('Copy -', k='hyphen', s=(15,2))], 
...
while True:
    ...
    if event in (sg.WINDOW_CLOSED, 'Exit'):
        ...
        if values['file_list'] == '':
            ...
            if len(new_names) > len(curr_names) or len(new_names) < len(curr_names):
                sg.popup('Number of new filenames does not match number of filenames from original list.', keep_on_top=True)
            else:
